      ...
      <el-card>
        ...
        <div class="cardBody">
          ...
          <div class="baseData clearFloat">
            ...
            <ul class="rightFloat">
              <li v-for="item in asnUdfDs">
                ...
                <span :title="params[item.paramCode]">&nbsp;{{params[item.paramCode]}}</span>
              </li>
            </ul>
          </div>
          <div class="mainData clearFloat">
            ...
            <ul class="leftFloat">
              <li>
                <span>&nbsp;{{params.expectQuantity}}</span>
                <span>预约:</span>
              </li>
            </ul>
            <ul class="rightFloat">
              <li>
                <span>&nbsp;{{params.packDescribe}}</span>
                <span>单位:</span>
              </li>
            </ul>
          </div>
          <div class="minorData">
            ...
            <ul>
              <li>
                <span>货主:</span>
                <span :title="params.organizationName">&nbsp;{{params.organizationName}}</span>
              </li>
              <li>
                <span>供应商:</span>
                <span :title="params.supplierName">&nbsp;{{params.supplierName}}</span>
              </li>
            </ul>
          </div>
        </div>
...
<!--      <div v-if="detailIndex !== null">-->
        ...
        <el-form-item label="追踪号" prop="toBoxCode" :label-width="formLabelWidth">
          <el-input v-model="form.toBoxCode" size="mini" placeholder="请输入追踪号" clearable disabled style="width: 90%;"></el-input>
        </el-form-item>
        ...
        <el-form-item label="包装描述" prop="toPackDetailId" :label-width="formLabelWidth">
          <el-select v-model="form.toPackDetailId" size="mini" filterable placeholder="请选择包装描述" style="width: 90%;">
            <el-option
        ...
        <el-form-item label="货位" prop="toCellCode" :label-width="formLabelWidth">
          <el-select v-model="form.toCellCode" size="mini" filterable placeholder="请选择包装描述" style="width: 90%;">
            <el-option
              v-for="item in cells"
              :key="item.cellCode"
              :label="item.cellName"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectItem(val){
        // let list = PF.JSON(this.asnDetails[val]);
        if(!PF.isNull(this.params)){
          this.$router.push({name: 'home'});
          return false
        }
        let list = PF.JSON(this.params);

        list.wareId = list.wareId;
        list.wareName = list.wareName;
        list.toCellCode = list.toCellCode;
        list.toQuantity = list.expectQuantity;
        list.toPackDetailId = list.packDetailId;
        this.form = list;
            ...
            for (let i = 0; i < batchTacticDetails.length; i++) {
              if (batchTacticDetails[i].state === 1) {
                ...
                if (batchTacticDetails[i].detailCode === 'detailNo') {
                  this.form.asnDetailNo = this.form.detailNo;
