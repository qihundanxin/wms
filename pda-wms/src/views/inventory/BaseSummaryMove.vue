<template>
    <div v-loading="loading" class="BaseSummaryMove">
      ...
      <div class="baseAcceptBillBox noScrollbar">
        <ul v-infinite-scroll="lazy">
            <li v-for="(item,index) in inventoryBalances" @click="BaseSummaryMove2(item)">
            <el-card> 
              <div class="cardBody">
  ...
  export default {
    ...
    methods: {
        ...
        if (item.cellUseType == -1) {
          IOT.tips('此货位不可进行汇总移库操作', 'error');
        } 
        if(item.cellUseType==3){
           IOT.tips('警告，集货位不可操作汇总移库！','warning',2000)
         }
        else {
          this.$router.push({ name: 'baseSummaryMove2', params: item });
        }
      },
      getInventoryBalanceData() {
        ...
        //原基础移库
        IOT.getServerData('/inventorys/summaryMoveList', 'post', searchData, (ret) => {
          this.loading = false;
          if (ret.code === 200) {
