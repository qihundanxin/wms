<template>
    <div class="freezeCell2">
        ...
        <div style="text-align: center;">
            ...
            <el-button @click="commitForm('form')" type="primary" size="mini" round icon="iconfont icon-quanxian"
                       style="width: 100px;margin-top: 5px;right: auto"> 解冻
            </el-button>
        </div>
                    ...
                    <li  v-for="(item,index) in inventoryBalances">
                        <el-card class="clearFloat">
                            ...
                            <div class="cardBottomBtn clearFloat">
                                ...
                                <div class="rightFloat" @click="commitFreeze(item)">解冻</div>
                            </div>
                        </el-card>
                    </li>
                </ul>
    ...
    export default {
        ...
        methods: {
            getInventoryBalanceData() {
                ...
                IOT.getServerData('/inventorys/findByRelease', 'get', searchData, (ret) => {
                    this.loading = false;
                    if (ret.code === 200) {
            ...
            commitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        IOT.getServerData('/inventorys/releaseCells', 'post', form, (ret) => {
                            IOT.hideOverlay();
                            if (ret.code === 200) {
            ...
            commitFreeze(row){
                if(!PF.isNull(row.freezeQuantity1) || row.freezeQuantity1 === 0){
                    IOT.tips('请输入解冻数！', 'error');
                    return false
                }
                IOT.showOverlay('保存中，请稍等...');
                IOT.getServerData('/inventorys/releaseCell', 'get', {inventoryId: row.inventoryId,freezeQuantity1: row.freezeQuantity1}, (ret) => {
                    IOT.hideOverlay();
                    if (ret.code === 200) {
                        IOT.tips('保存成功！', 'success', 1000, () => {
                            this.searchData.pageSize = 2;
