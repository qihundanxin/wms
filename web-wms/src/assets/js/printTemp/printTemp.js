		...
		drop:function(event,ui){
			...
			switch(labeltype){
			   ...
			   /* case "declarenewlist":
				...
				//创建本地上传的图片
				//case "onlineimage":$(".custom-drop").append('<div class="dropitem character onlineimage" data-type="'+labeltype+'" data-title="'+ui.draggable.find(".title").text()+'" style="width:50px;height:50px;left:'+positionLeft+'px;top:'+positionTop+'px"><span></span><img src="https://global.mabangerp.com/image/photo_default.jpg"></div>');break;
				case "uploadimage":$(".custom-drop").append('<div class="dropitem character onlineimage" data-type="'+labeltype+'" data-title="'+ui.draggable.find(".title").text()+'" style="width:50px;height:50px;left:'+positionLeft+'px;top:'+positionTop+'px"><span></span><img src="https://global.mabangerp.com/image/photo_default.jpg"></div>');break;
				//创建预设图片
	...
	function getTempSet(obj){
		...
		switch(obj.attr("data-type")){
			...
			case "onlineimage":
				...
				image.attr("src")=="https://global.mabangerp.com/image/photo_default.jpg"?$("#imageUrl").val(""):$("#imageUrl").val(image.attr("src"));
				//返回边框设定
				setBorder(obj);
			break;
			//本地上传图片
...
// 			});
	...
	$("#templetfile").on("change",function(){
		if (window.File && window.FileList) {
			...
			$.ajaxFileUpload(
				{
					url: 'https://www.mabangerp.com/index.php?mod=printtemplate.addimage',
					//url: 'https://www.mabangerp.com/index.php?mod=company.doUpdateCompanyLogo',
					secureuri: false,
...
function doSavePrintTemplate(ctrl,type)
{
	...
	$.ajax({
		type: "POST",
		url: "https://www.mabangerp.com/index.php?mod=printtemplate.doSave&isAdmin=&companyId=193923&signs=&firstPageRow=" + maxNum + "&nextPageRow=" + moreNum + '&headHeight=' + headHeight + '&footHeight=' + footHeight + '&taxRatio='+taxRatio,
		data: $form.serialize() ,
		dataType: 'json',
		...
		success: function(r){
			...
			if(r.success) {	
				...
				if(type == 2){
					window.location.href="https://www.mabangerp.com/index.php?mod=printtemplate.list&companyId=193923&signs=&type=2";
				}
			}
			else {
				$.gritter.add({
...
/*
	...
	*/
function doPreviewTemplate(ctrl)
{
	var url = 'https://www.mabangerp.com/index.php?mod=printtemplate.preview&id=104921';
	var n = window.open('https://www.mabangerp.com/index.php?mod=main.loading&url=' + encodeURIComponent(url), '_blank');
	doSavePrintTemplate(ctrl);
	//window.setTimeout(n.window.location.href="", 500);
}
