<template>
  <div   v-loading="loading" class="asnBillDetail">
    ...
    <el-dialog title="修改单据" :visible.sync="dialogNewVisible" :before-close="handleClose" width="900px"
      ...
      <div class="bigFormBox2">
        <el-form class="clearFloat" :model="form.asnMaster" :rules="rules" ref="form.asnMaster">
          ...
          <el-form-item label="单据类型" prop="asnType" :label-width="formLabelWidth">
            <el-select v-model="form.asnMaster.asnType" filterable placeholder="请选择单据类型" style="width: 70%;">
              <el-option
    ...
    <el-dialog title="生成 QC 单据" :visible.sync="dialogQcVisible" :before-close="handleClose2" width="900px"
      ...
      <div class="bigFormBox2">
        <el-form class="clearFloat" :model="formQc.qcMaster" :rules="rulesQc" ref="formQc.qcMaster">
          <el-form-item label="单据号" prop="asnBillNo" :label-width="formLabelWidth">
            <el-input v-model="formQc.qcMaster.asnBillNo" placeholder="请输入单据号" disabled></el-input>
          </el-form-item>
    ...
    <el-dialog title="单据验收" :visible.sync="dialogAcceptVisible" :before-close="handleClose4" width="900px" style="margin-top: -10vh;">
        ...
        <el-form class="clearFloat" :model="formAccept" :rules="rulesAccept" ref="formAccept">
          ...
          <el-form-item label="验收数量" prop="toQuantity" :label-width="formLabelWidth">
            <el-input-number v-model="formAccept.toQuantity" controls-position="right" :min="0"
                             label="请输入数量"
                             style="width: 90%;" @change="changePackDetailId(0,'accept')" ></el-input-number>
          </el-form-item>
          <el-form-item label="验收数量" prop="toQuantity" :label-width="formLabelWidth">
          <span style="color:red">{{formAccept.toQuantity}}（主单位） </span>
          </el-form-item>          
          ...
          <el-form-item label="货位编码" prop="toCellCode" :label-width="formLabelWidth">
            <el-input  v-model="formAccept.toCellCode" placeholder="请输入编码" style="width: 80%;"></el-input>
    ...
    <div class="publicSearch">
      <div class="searchBar" :style="{height: isMoreSearchMaster?'auto':''}">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="width: 100%;">
          <el-tab-pane :label="isFile?'单归档单据信息':'单据信息'" name="first">
            <div class="newBillData clearFloat">
              <ul class="leftFloat">
                ...
                <li v-if="asnMaster.soBillNo">
                  <span>退货来源:</span>
                  <span :title="asnMaster.soBillNo" class="textBtn"
                        @click="$router.push({name: 'soBillDetail',params: {billNo: asnMaster.soBillNo}})">{{asnMaster.soBillNo}}</span>
                </li>
  ...
  export default {
    ...
    methods: {
        ...
        this.loading = false;
          if (ret.code === 200) {
            ...
            if (PF.isNull(list.createTime)) {
              list.createTime1 = list.createTime.substr(0, 16);
            }
            this.asnMaster = list;
          } else {
            IOT.tips(ret.message || '服务器请求失败，稍后再试！', 'error');
      ...
      getAsnRecordData() {
        ...
        IOT.getServerData('/accept/records/list', 'get', {asnBillNo: this.searchData.billNo}, (ret) => {
        ...
        if (type === 'qc') {
          ...
          for (let i = 0; i < this.packDetails.length; i++) {
            if (this.packDetails[i].packDetailId === this.formAccept.toPackDetailId) {
              ...
              this.acceptQuantity2 = this.formAccept.toQuantity*this.formAccept.toTransRatio
            }
          }
          // this.form.asnDetails[index].expectQuantity = PF.changePToChangeQ(this.form.asnDetails[index]);
        }
      ...
      showAcceptModal(row) {
        console.log("-------showAcceptModal-------")
        console.log(this.formAccept);
        ...
        this.acceptQuantity2=0
        this.$set(this.formAccept,"toQuantity",0)
        //验收数量，默认值
      ...
      selectCell(cellCode) {
       ...
       commitAccept ( ) {
        ...
        if(form.toPackDetailId==null){
          IOT.tips('请选择单位！', 'error', 1000, () => {
              IOT.hideOverlay();
            });
            return false
        }
        if (form.toQuantity * 1 % 1 !== 0) {
          IOT.tips('请填写整数！', 'error', 1000, () => {
              IOT.hideOverlay();
            });
            return false
        }
        if (form.toQuantity === 0) {
            IOT.tips('请填写验收数量！', 'error', 1000, () => {
              IOT.hideOverlay();
            });
            return false
        }
        if(form.toCellCode == '' || form.toCellCode ==undefined || form.toCellCode ==null){
          IOT.tips('请输入货位编码！', 'error', 1000, () => {
            IOT.hideOverlay();
          });
          return false
        }
        ...
        IOT.getServerData('/accept/records/accept', 'post', form, (ret) => {
          IOT.hideOverlay();
          if (ret.code === 200) {
