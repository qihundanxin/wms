  ...
  <div class="createAsnBill">
    ...
    <div class="publicSearch bigFormBox3">
      <div class="searchBar" :style="{ height: isMoreSearch ? 'auto' : '' }">
        <p style="font-size: 20px;font-weight: 600;line-height: 20px;margin-top: 15px;">入库单信息</p>
        <el-form class="clearFloat" :model="form.asnMaster" :rules="rules" ref="form.asnMaster">
          ...
          <el-form-item label="入库单类型" prop="asnType" :label-width="formLabelWidth">
            <el-select v-model="form.asnMaster.asnType" filterable placeholder="请选择入库单类型" size="small"
              style="width: 100%;">
      ...
      <div class="searchBar" :style="{ height: isMoreSearch ? 'auto' : '' }">
        ...
        <el-form class="clearFloat" :rules="rules" ref="form.soMaster">
          <el-form-item label="承运方" :label-width="formLabelWidth">
            ...
            <el-select v-model="form.shipCode" filterable placeholder="请选择承运方" size="small" style="width: 100%;">
          ...
          <el-form-item label="物流单号" :label-width="formLabelWidth">
            <el-input v-model="form.shipBillCode" placeholder="请物流单号" size="small"></el-input>
          </el-form-item>
          <br />
        </el-form>
      </div>
    ...
    <div class="tableBox">
      <el-table highlight-current-row :data="form.asnDetails" style="width: 100%">
        ...
        <el-table-column label="入库单数量" width="180">
          <template slot-scope="scope">
...
export default {
          ...
          form.asnShipDetails = [
            {
              ...
              shipBillCode: this.form.shipBillCode
            }
          ]
          IOT.getServerData('/asn/masters/insert', 'post', form, (ret) => {
            IOT.hideOverlay();
            if (ret.code === 200) {
              PF.removeLocal('CreateAsnBill:Form:' + this.wareId);
  ...
  mounted() {
    this.oFormJson = JSON.stringify(this.form);
    let setTimer = () => {
      this.autoSaveCacheTimer = setInterval(() => {
        if (this.oFormJson != JSON.stringify(this.form)) {
          PF.setLocal('CreateAsnBill:Form:' + this.wareId, this.form);
        } else {
          PF.removeLocal('CreateAsnBill:Form:' + this.wareId);
        }
      }, 10 * 1000);
    }
    let cacheForm = PF.getLocal('CreateAsnBill:Form:' + this.wareId, 'json');
    if (cacheForm) {
      this.$confirm('之前有未保存的入库单，是否恢复编辑？')
        .then(_ => {
          this.form = cacheForm;
          setTimer();
        })
  ...
  beforeDestroy() {
    if (this.autoSaveCacheTimer) {
      clearInterval(this.autoSaveCacheTimer);
    }
  }
}
</script>
