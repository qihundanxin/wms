  ...
  <div class="buyBillIn">
    <div class="header">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>入库日志</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div class="tableBox table-500h">
      <el-table
          ...
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <el-form-item label="采购单号">
                <h4>{{ props.row.billNo }}</h4>
              </el-form-item>
              <el-form-item label="仓位名称">
                <h4>{{ props.row.cellCode }}</h4>
              </el-form-item>
              <el-form-item label="采购量">
                <h4>{{ props.row.expectQuantity }}</h4>
              </el-form-item>
              <el-form-item label="仓库名称">
                <h4>{{ props.row.wareName }}</h4>
              </el-form-item>
              <el-form-item label="入库量">
                <h4>{{ props.row.quantity }}</h4>
              </el-form-item>
            </el-form>
          </template>
        </el-table-column>
        ...
        >
          <template slot-scope="scope">
            <div>{{ scope.row.itemName }} / {{ scope.row.batchName }} / {{ scope.row.itemCode }}</div>
          </template>
        </el-table-column>
        <el-table-column
            show-overflow-tooltip
        ...
        >
          <template slot-scope="scope">
            <div>{{ parseState(scope.row.billType) }} / {{ scope.row.acceptType == 1 ? '完成' : '待上架' }}</div>
          </template>
        </el-table-column>
        <el-table-column
            show-overflow-tooltip
        ...
        >
          <template slot-scope="scope">
            {{ scope.row.acceptState == 1 ? '同步erp失败' : '同步erp成功' }}
          </template>
        </el-table-column>
        <el-table-column
            show-overflow-tooltip
        ...
        >
          <template slot-scope="scope">
            <div>
              {{ scope.row.acceptUserName }} / {{ scope.row.acceptTime | timestamp2str }}
            </div>
          </template>
        </el-table-column>
        <el-table-column
...
export default {
  ...
  methods: {
    ...
    getBuyBillInData() {
      ...
      // }
      IOT.getServerData('/accept/log/list', 'post', searchData, (ret) => {
