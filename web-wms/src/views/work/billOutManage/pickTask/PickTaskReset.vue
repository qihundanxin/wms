  ...
  <div class="pickTask">
    <div class="publicSearch">
      <div class="searchBar" :style="{ height: isMoreSearch ? 'auto' : '' }">
        ...
        <div>
          <span>出库单编号</span>
          <el-input style="width:160px" v-model="searchData.billNo" size="mini" placeholder="请输入出库单编号"
            clearable></el-input>
        </div>
        <div>
          <span>SKU</span>
        ...
        <div>
          <span>货位编码/名称</span>
          <el-input style="width:160px" v-model="searchData.cellCode" size="mini" placeholder="请输入货位编码/名称"
            clearable></el-input>
        </div>
        <div>
          <span>拣货状态</span>
          <el-select v-model="searchData.state" size="mini" filterable placeholder="请选择拣货状态" clearable>
            <el-option v-for="item in pickTaskType" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </div>
        <!-- <div>
    ...
    <el-dialog title="拣货确认" :visible.sync="dialogNewVisible" :before-close="handleClose" width="1000px"
      ...
      <div class="upperTacticBox">
        <el-form class="searchBar" :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="目标货位" prop="toCellCode" :label-width="formLabelWidth">
            <el-select v-model="form.toCellCode" filterable placeholder="请选择目标货位" size="small" style="width: 70%;">
              <el-option v-for="item in cells" :key="item.cellCode" :label="item.cellCode" :value="item.cellCode">
              </el-option>
            </el-select>
          </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button style="margin-right: 550px;" type="primary" @click="showSerialNo()"
          :disabled="serialNo1.length <= 0">序列号({{ serialNo1.length }})</el-button>
      ...
      <div class="cellModalBox">
        <el-form class="clearFloat" :model="form" :rules="rules" ref="form">
          <el-form-item label="扫描输入框" :label-width="formLabelWidth">
            <el-input v-model="scanner" id="scanner" @change="addSerialNo()" placeholder="请扫描序列号"
          ...
          <el-form-item label="是否自动提交">
            <el-switch v-model="select" active-color="#13ce66" inactive-color="#ff4949" active-text="是" inactive-text="否"
          ...
          <el-tag v-for="(item, i) in serialNo1" :key="i" :style="i === 0 ? '' : 'margin-left: 10px;;margin-top:10px;'"
    ...
    <div class="tableBox">
      <div style="margin:10px">
        <el-button type="primary" size="small" :disabled="canReset" @click="pickTaskResetShow">重置分配数</el-button>
      </div>
      <el-table @selection-change="handleSelectionChange" v-loading="loading.masterTable" highlight-current-row
        :data="pickTasks" style="width: 100%">
        <el-table-column type="selection" :selectable='selectEnable' width="55">
        </el-table-column>
        <el-table-column fixed="left" type="index" label="序号" width="50">
        </el-table-column>
        <el-table-column prop="pickTaskId" label="拣货任务id" min-width="70">
        </el-table-column>
        <el-table-column prop="billNo" label="出库单编号" min-width="110">
        </el-table-column>
        <el-table-column show-overflow-tooltip prop="itemCode" label="SKU/SKU名称" min-width="150">
        ...
        <el-table-column show-overflow-tooltip prop="pickQuantity" label="分配商品数量" min-width="80" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.pickQuantity }}</span>
          </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip prop="soCreateTime" label="订单创建时间" min-width="100">
        </el-table-column>
        <el-table-column show-overflow-tooltip label="销售状态" min-width="70">
          <template slot-scope="scope">
            <el-tag>未完成</el-tag>
          </template>
        </el-table-column>
        <el-table-column show-overflow-tooltip label="订单编号" prop="allotBillNo" min-width="70">
        </el-table-column>
        <el-table-column sortable show-overflow-tooltip prop="cellCode" label="货位" min-width="100">
          <template slot-scope="scope">
        ...
        <el-table-column sortable show-overflow-tooltip prop="state" label="状态" min-width="80">
          <template slot-scope="scope">
            <el-tag v-for="(item, index) in pickTaskType" :key="index" v-if="scope.row.state === item.value">{{ item.label
            }}</el-tag>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination class="pagination-bottom" style="text-align: right;" @current-change="handleCurrentChange"
        :current-page.sync="searchData.pageNum" :page-size="searchData.pageSize" @size-change="handleSizeChange"
        :page-sizes="[10, 30, 50, 100]" layout="sizes,total, prev, pager, next, jumper" :total="searchData.total">
      </el-pagination>
    </div>
    <!-- 重置货位弹框 -->
    <el-dialog title="重置货位" :visible.sync="pickResetVisible" width="60%" style="margin-top: -10vh;height:900px" center>
      <el-table :data="pickResetData" border style="width: 100%" max-height="500">
        <el-table-column type="index" label="序号" width="100">
        </el-table-column>
        <el-table-column prop="itemCode" label="sku" min-width="100">
        </el-table-column>
        <el-table-column prop="itemName" label="sku名称" min-width="200">
        </el-table-column>
        <el-table-column prop="cellCode" label="当前货位" min-width="100">
        </el-table-column>
        <el-table-column prop="pickQuantity" label="重置分配数" min-width="80">
        </el-table-column>
      </el-table>
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="pickTaskReset">确认重置</el-button>
        <el-button @click="pickResetVisible = false">取 消</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    pickTaskResetShow() {
      ...
      for (var i = 0; i < this.multipleSelection.length; i++) {
        ...
        this.pickTaskIds.push(item.pickTaskId);
        let key = {
      ...
      map.forEach((value, key) => {
        ...
        this.pickResetData.push(item);
      })
      this.pickResetVisible = true;
    },
    //确认重置--重置分配数
    pickTaskReset() {
      ...
      IOT.getServerData('/pick/tasks/pickTaskReset', 'post', data, (ret) => {
        IOT.hideOverlay();
        loading.close();
        if (ret.code === 200) {
          IOT.tips('货位重置成功', 'success', 1000, () => {
            this.pickResetVisible = false;
    ...
    commitCancel() {
      ...
      for (let i = 0; i < form.length; i++) {
        if (form[i].state === 3) {
          IOT.tips('请选择未取消的拣货任务！', 'error', 1000, () => {
            s
            IOT.hideOverlay();
          });
          return false
