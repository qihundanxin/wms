<template>
  <div class="asnBillDetail">
    ...
    <div class="headerBtns">
      ...
      <el-button v-if="asnMaster.insertType === -2" size="small" type="info" round icon="iconfont icon-tijiao"
                 @click="showUpdateModal"> 修改
      </el-button>
      <el-button v-if="asnMaster.insertType === -2" size="small" type="success" round icon="iconfont icon-xinzeng"
                 @click="isCommit()"> 提交
      </el-button>
      <el-button size="small" type="danger" round icon="iconfont icon-shanchu" @click="isDelete(asnMaster.asnMasterId)">
        删除
      </el-button>
      <el-button v-if="!isFile && asnMaster.insertType === 0 && asnMaster.qcState === -1" size="small" type="warning" round icon="iconfont icon-qc"
                 @click="showQcModal"> 质检
      </el-button>
      <el-button v-if="(asnMaster.state === 0 || asnMaster.state === 2) && formReview.caseDetailId !== null" size="small" type="warning" round
                 icon="iconfont icon-shenhe" @click="dialogReviewVisible = true"> 审核
      </el-button>
    </div>

    <el-dialog title="修改入库单单" :visible.sync="dialogNewVisible" :before-close="handleClose" width="900px"
    ...
    <el-dialog title="生成 QC 单据" :visible.sync="dialogQcVisible" :before-close="handleClose2" width="900px"
      ...
      <div class="bigFormBox2">
        <el-form class="clearFloat" :model="formQc.qcMaster" :rules="rulesQc" ref="formQc.qcMaster">
          <el-form-item label="入库单号" prop="asnBillNo" :label-width="formLabelWidth">
            <el-input v-model="formQc.qcMaster.asnBillNo" placeholder="请输入入库单号" disabled></el-input>
          </el-form-item>
    ...
    <el-dialog title="入库单验收" :visible.sync="dialogAcceptVisible" :before-close="handleClose4" width="900px" style="margin-top: -10vh;">
      <div class="cellModalBox">
        <el-form class="clearFloat" :model="formAccept" :rules="rulesAccept" ref="formAccept">
          <el-form-item label="仓库" prop="wareId" :label-width="formLabelWidth">
            <el-input :title="formAccept.wareName" v-model="formAccept.wareName" placeholder="仓库" disabled style="width: 90%;"></el-input>
          </el-form-item>
          <el-form-item label="货主" prop="organizationId" :label-width="formLabelWidth">
            <el-input :title="formAccept.organizationName" v-model="formAccept.organizationName" placeholder="货主" disabled style="width: 90%;"></el-input>
          </el-form-item>
          <el-form-item label="SKU名称" prop="itemName" :label-width="formLabelWidth">
            <el-input :title="formAccept.itemName" v-model="formAccept.itemName" placeholder="SKU名称" disabled style="width: 90%;"></el-input>
          </el-form-item>
          <el-form-item label="SKU" prop="itemCode" :label-width="formLabelWidth">
            <el-input :title="formAccept.itemCode" v-model="formAccept.itemCode" placeholder="SKU" disabled style="width: 90%;"></el-input>
          </el-form-item>
          <el-form-item label="预约数量" prop="expectQuantity" :label-width="formLabelWidth">
            <span>{{formAccept.expectQuantity}}</span>
          </el-form-item>
          <el-form-item label="已验收数量" prop="acceptQuantity" :label-width="formLabelWidth">
            <span>{{formAccept.acceptQuantity}}</span>
          </el-form-item>
...
<!--        <el-button type="primary" @click="commitBatchTactic('form')">确 定</el-button>-->
        <el-button type="primary" @click="commitAcceptList('accept')">验 收</el-button>
      </div>
    </el-dialog>
      ...
      <div class="tableBox">
        <el-table
          ...
          <el-table-column
            ...
            width="180">
            <template slot-scope="scope">
              <span>{{scope.row.palletLimit.toFixed(2)}}</span>
            </template>
          </el-table-column>
          <el-table-column
            sortable
    ...
    <div class="publicSearch">
      <div class="searchBar" :style="{height: isMoreSearchMaster?'auto':''}">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick" style="width: 100%;">
              ...
              <ul class="leftFloat">
                ...
                <li>
                  ...
                  <span :title="asnMaster.createUserName" class="textBtn"
                        @click="$parent.showPublicModal(asnMaster.userId, 'account')">{{asnMaster.createUserName}}</span>
                </li>
                <li>
                  <span>创建时间:</span>
          ...
          <el-tab-pane v-if="asnMaster.insertType === 0" label="审核进程" name="second">
    ...
    <el-tabs type="card" v-model="activeName2" @tab-click="handleClick">
      ...
      <el-tab-pane label="验收记录" name="second">

      </el-tab-pane>
      <el-tab-pane label="上架任务" name="third">
    ...
    methods: {
        ...
        IOT.getServerData(url, 'get', {billNo: this.searchData.billNo}, (ret) => {
          if (ret.code === 200) {
            ...
            if (PF.isNull(list.expectTime)) {
              list.expectTimeStr = DateUtils.dateWithoutYear(list.expectTime);
            }
            this.asnMaster = list;
          } else {
        ...
        IOT.getServerData(url, 'get', this.searchData, (ret) => {
          if (ret.code === 200) {
            ...
            for(let i=0;i<list.length;i++){
              ...
              list[i].oldQ = list[i].expectQuantity;
      ...
      commitAcceptList(type) {
        ...
        for (let i = 0; i < form.length; i++) {
          ...
          // 主单位数量
          form[i].toQuantity = form[i].toQuantity * form[i].toTransRatio;
        }
